...
#include "BuddhaJuliaShaderMP.h"

BuddhaJuliaShaderMP::BuddhaJuliaShaderMP(void) {
	_functionName = std::string("fnJuliaBrotShader");
	_buddhaBuffer = NULL;
	_redraw = true;
} 


bool BuddhaJuliaShaderMP::initializeShader() {
	
	std::cout << "BuddhaJuliaShaderMP::initializeShader" << std::endl;
...
void BuddhaJuliaShaderMP::setRenderSize(unsigned int width, unsigned int height) {

	width = width + 4 - (width & 3);
	
	std::cout << "BuddhaJuliaShaderMP::setRenderSize "  << width << ", " << height << std::endl;
...
bool BuddhaJuliaShaderMP::preRenderInitialization(unsigned int shaderProgram) {

	
	std::cout << "BuddhaJuliaShaderMP::preRenderInitialization "  << _textureWidth << ", " << _textureHeight << std::endl;
...
bool BuddhaJuliaShaderMP::postRendertidyUp(unsigned int shaderProgram) {

	return true;

}

void BuddhaJuliaShaderMP::createShaderCode() {
...
void BuddhaJuliaShaderMP::renderBuddha() {
	...
	complex c, z, z0;
        unsigned int maxIter = 28000;
	double maxIterDouble = 70550.0 / (double)maxIter ;	
...
//	c = complex(0.5, 0.7);
//	c = complex(0.286, 0.01);
	c = complex(0.001, 0.001);

//#pragma omp parallel for ordered schedule(static) private(i, j, x, z, z0, xPlot, yPlot, c)
//#pragma omp for ordered schedule(static) private(i, j, x, z, z0, xPlot, yPlot, c)
#pragma omp parallel for ordered schedule(dynamic) private(i, j, x, z, z0, xPlot, yPlot, c)

        for(unsigned int y=0; y<_textureHeight * 2; y++) {
		for(x=0; x<_textureWidth * 2; x++) {
...
void BuddhaJuliaShaderMP::render(double top, double bottom, double left, double right) {

//	sleep(10);
	
	std::cout << "BuddhaJuliaShaderMP::render "  << _textureWidth << ", " << _textureHeight << std::endl;
...
std::cout << "BuddhaJuliaShaderMP::render finished mandlebrot" << std::endl;
	...
	std::cout << "BuddhaJuliaShaderMP::finished" << std::endl;
	
}


std::string &BuddhaJuliaShaderMP::functionName(void) {
